...
*By [@mattganzak](https://twitter.com/mattganzak) — ScaleUP Media*
...
Update your config at `~/.openclaw/openclaw.json`:
...
**THE PROBLEM:** Your system prompt, workspace files (SOUL.md, USER.md), and reference materials get sent to the API with every single message. If your system prompt is 5KB and you make 100 API calls per week, that's 500KB of identical text being re-transmitted and re-processed every week.
...
**1. Batch requests within 5-minute windows** — Make multiple API calls in quick succession to reduce cache misses.

**2. Keep system prompts stable** — Don't update SOUL.md mid-session. Changes invalidate cache; batch them during maintenance windows.

**3. Organize context hierarchically** — Core system prompt (highest priority), then stable workspace files, then dynamic daily notes (uncached).

**4. For projects: Separate stable from dynamic** — Use `product-reference.md` (stable, cached) and `project-notes.md` (dynamic, uncached).

### Real-World Example: Outreach Campaign

Running 50 outreach email drafts per week using Sonnet:
...
*Questions? DM [@mattganzak](https://twitter.com/mattganzak)*
